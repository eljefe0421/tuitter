...
export class OAuthSession {
    ...
    async ensureValidTokens(forceRefresh = false) {
    ...
    async ensureValidTokensInternal(forceRefresh) {
        if (!this.tokens) {
            this.tokens = await this.loadStoredTokens();
        }
        const missingScopes = this.tokens ? this.getMissingScopes(this.tokens) : [];
        if (missingScopes.length > 0) {
            console.log(`Cached OAuth token missing scopes: ${missingScopes.join(", ")}. Re-authenticating with requested scopes...`);
    ...
    getMissingScopes(tokens) {
        ...
        const granted = new Set(tokens.scope ? tokens.scope.split(/\s+/).filter(Boolean) : tokens.requestedScopes?.filter(Boolean));
        return this.config.scopes.filter((scope) => !granted.has(scope));
    }
    isExpiringSoon(tokens) {
    ...
    async runAuthorizationCodeFlow() {
        ...
        const tokenResponse = await this.requestToken(new URLSearchParams({
    ...
    async waitForCallbackCode(redirectUrl, expectedState) {
